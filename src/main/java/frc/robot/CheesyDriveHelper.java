...
* Effectively makes the robot execute 'constant-radius' turning, where 
the radius of curvature is independent of speed.
* Returns a 'DriveSignal', which is a helper class to encapsulate motor 
commands.
 */
...
public class CheesyDriveHelper {
    ...
    // These factor determine how fast the wheel traverses the "non 
linear" sine curve.
    ...
    public DriveSignal cheesyDrive(double throttle, double wheel,
        ...
        if (isHighGear) {
            ...
            final double denominator = Math.sin(Math.PI / 2.0 * 
wheelNonLinearity);
            // Apply a sin function that's scaled to make it feel 
better.
...
/ denominator;
        ...
        if (isHighGear) {
            ...
            if (wheel * negInertia > 0) {
                // If we are moving away from 0.0, aka, trying to get 
more wheel.
